package me.ljseokd.studyspringbootwebapp.event.validator;
...
public class EventValidator implements Validator {
    ...
    public void validate(Object o, Errors errors) {
        ...
        if (eventForm.getEndEnrollmentDateTime().isBefore(LocalDateTime.now())){
            errors.rejectValue("endEnrollmentDateTime", "wrong.datetime", "모입 접수 종료 일시를 정확히 입력하세요.");
        }

        if (isNotValidEndDateTime(eventForm)){
            errors.rejectValue("endDateTime", "wrong.datetime", "모입 접수 종료 일시를 정확히 입력하세요.");
        }

        if (isNotValidStartDateTime(eventForm)){
            errors.rejectValue("startDateTime", "wrong.datetime", "모입 접수 종료 일시를 정확히 입력하세요.");
        }
